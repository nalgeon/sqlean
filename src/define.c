...
static void destroy_function(void* ctx) {
    sqlite3_free(ctx);
}

static void create_function(sqlite3_context* ctx, int argc, sqlite3_value** argv) {
    int ret;
    char* sql = sqlite3_mprintf("select %s", sqlite3_value_text(argv[1]));
    if (!sql) {
        ret = SQLITE_NOMEM;
        goto error;
    }
    ...
    if ((ret = sqlite3_prepare_v3(db, sql, -1, SQLITE_PREPARE_PERSISTENT, &stmt, NULL)) !=
        SQLITE_OK) {
        sqlite3_free(sql);
        goto error;
    }
    ...
    if ((ret = sqlite3_create_function_v2(db, (const char*)sqlite3_value_text(argv[0]), nparams,
                                          SQLITE_UTF8, sql, call, NULL, NULL, destroy_function)) !=
        SQLITE_OK)
        goto error;
    return;
...
static int statement_vtab_create(sqlite3* db,
                                 void* pAux,
                                 int argc,
    ...
    int ret;
    sqlite3_mutex* mutex =
        sqlite3_db_mutex(db);  // only needed to ensure correctness of sqlite3_errmsg
...
// if these point to the literal same function sqlite makes statement_vtab eponymous, which we don't
// want
static int statement_vtab_connect(sqlite3* db,
                                  void* pAux,
                                  int argc,
                                  const char* const* argv,
                                  sqlite3_vtab** ppVtab,
...
static int statement_vtab_filter(sqlite3_vtab_cursor* cur,
                                 int idxNum,
                                 const char* idxStr,
                                 int argc,
                                 sqlite3_value** argv) {
...
__declspec(dllexport)
    ...
    sqlite3_create_function(db, "create_function", 2, flags, NULL, create_function, NULL, NULL);
    sqlite3_create_module(db, "statement", &statement_vtab_module, NULL);
    return SQLITE_OK;
}
