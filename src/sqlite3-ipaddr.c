...
static void sqlite3_ipfamily(sqlite3_context *context, int argc, sqlite3_value **argv) {
    assert(argc == 1);
...
static void sqlite3_iphost(sqlite3_context *context, int argc, sqlite3_value **argv) {
    assert(argc == 1);
    if (sqlite3_value_type(argv[0]) == SQLITE_NULL) {
        sqlite3_result_null(context);
        return;
...
static void sqlite3_ipmasklen(sqlite3_context *context, int argc, sqlite3_value **argv) {
    assert(argc == 1);
    if (sqlite3_value_type(argv[0]) == SQLITE_NULL) {
        sqlite3_result_null(context);
        return;
...
static void sqlite3_ipnetwork(sqlite3_context *context, int argc, sqlite3_value **argv) {
    assert(argc == 1);
    if (sqlite3_value_type(argv[0]) == SQLITE_NULL) {
        sqlite3_result_null(context);
        return;
    ...
    if (ip->af == AF_INET) {
        ...
        ip->ipv4.s_addr = htonl(ntohl(ip->ipv4.s_addr) & ~(uint32_t)((1ULL << (32 - ip->masklen)) - 1));
...
static void sqlite3_ipcontains(sqlite3_context *context, int argc, sqlite3_value **argv) {
    assert(argc == 2);
    ...
    if (ip1->af == AF_INET) {
        ip1->ipv4.s_addr = htonl(ntohl(ip1->ipv4.s_addr) & ~(uint32_t)((1ULL << (32 - ip1->masklen)) - 1));
        ip2->ipv4.s_addr = htonl(ntohl(ip2->ipv4.s_addr) & ~(uint32_t)((1ULL << (32 - ip1->masklen)) - 1));
...
#endif
    int sqlite3_ipaddr_init(
        sqlite3 *db,
        char **pzErrMsg,
        const sqlite3_api_routines *pApi) {
    SQLITE_EXTENSION_INIT2(pApi);
    static const int flags = SQLITE_UTF8 | SQLITE_INNOCUOUS | SQLITE_DETERMINISTIC;
    sqlite3_create_function(db, "ipfamily", 1, flags, 0, sqlite3_ipfamily, 0, 0);
    sqlite3_create_function(db, "iphost", 1, flags, 0, sqlite3_iphost, 0, 0);
