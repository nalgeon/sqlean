// Created by Richard Hipp, Public Domain
// https://sqlite.org/src/file/ext/misc/dbdump.c

// Converts the content of an SQLite database into UTF-8 text SQL statements
// that can be used to exactly recreate the original database.
// ROWID values are preserved.

// dbdump([schema[, table]])
// `schema` is the schema within that database which is to be dumped.
// Default is "main" but can also be "temp" or any ATTACH-ed database.
// If `table` is provided, then only the content of that one table is dumped.
// If `table` is omitted, then all tables are dumped.
...
static char** tableColumnList(DState* p, const char* zTab) {
    ...
    while (sqlite3_step(pStmt) == SQLITE_ROW) {
        ...
        if (sqlite3_column_int(pStmt, 5)) {
            nPK++;
            if (nPK == 1 &&
                sqlite3_stricmp((const char*)sqlite3_column_text(pStmt, 2), "INTEGER") == 0) {
                isIPK = 1;
            } else {
                isIPK = 0;
            }
                    ...
                    switch (sqlite3_column_type(pStmt, i)) {
                        ...
                        case SQLITE_TEXT: {
                            output_quoted_escaped_string(
                                p, (const char*)sqlite3_column_text(pStmt, i));
                            break;
                        }
                        case SQLITE_BLOB: {
...
*/
static void output_sql_from_query(DState* p,           /* Query context */
...
*/
static void run_schema_dump_query(DState* p, const char* zQuery, ...) {
    char* zErr = 0;
    char* z;
    va_list ap;
    va_start(ap, zQuery);
...
static int xCallback(const char* str, void* stream) {
    sqlite3_str_appendall((sqlite3_str*)stream, str);
    return 0;
}
...
** database.
*/
static DState db_dump(sqlite3* db, const char* zSchema, const char* zTable) {
    sqlite3_str* pArg = sqlite3_str_new(NULL);
    DState x;
    memset(&x, 0, sizeof(x));
...
static void sqlite3_dbdump(sqlite3_context* context, int argc, sqlite3_value** argv) {
    const char* schema = argc >= 1 ? (const char*)sqlite3_value_text(argv[0]) : "main";
    const char* table = argc == 2 ? (const char*)sqlite3_value_text(argv[1]) : NULL;
    sqlite3* db = sqlite3_context_db_handle(context);
    DState state = db_dump(db, schema, table);
    char* val = sqlite3_str_finish((sqlite3_str*)state.pArg);
    if (state.rc != 0) {
        sqlite3_result_error(context, "Error dumping data", -1);
        sqlite3_free(val);
    } else {
...
#endif
    int sqlite3_dbdump_init(sqlite3* db, char** errmsg_ptr, const sqlite3_api_routines* api) {
    (void)errmsg_ptr;
    SQLITE_EXTENSION_INIT2(api);
    static const int flags = SQLITE_UTF8 | SQLITE_INNOCUOUS | SQLITE_DETERMINISTIC;
    sqlite3_create_function(db, "dbdump", -1, flags, 0, sqlite3_dbdump, 0, 0);
    return SQLITE_OK;
}
