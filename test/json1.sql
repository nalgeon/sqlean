...
select '31', json_group_array(v) = '[1,2,3,4]' from (
  select 1 as v union select 2 union select 3 union select 4
);
select '32', json_group_object('v', v) = '{"v":1,"v":2,"v":3,"v":4}' from (
  select 1 as v union select 2 union select 3 union select 4
);
